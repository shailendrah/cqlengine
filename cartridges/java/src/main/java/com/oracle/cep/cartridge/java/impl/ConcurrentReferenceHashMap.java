...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    public ConcurrentReferenceHashMap(int initialCapacity, float loadFactor, int concurrencyLevel,
        ...
        assert (initialCapacity >= 0) : "Initial capacity must not be negative";
        assert (loadFactor > 0f) : "Load factor must be positive";
        assert (concurrencyLevel > 0) : "Concurrency level must be positive";
        assert (referenceType != null) : "Reference type must not be null";
    ...
    private V put(final K key, final V value, final boolean overwriteExisting) {
        return doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
            ...
            protected V execute(Reference<K, V> reference, Entry<K, V> entry, Entries entries) {
                ...
                assert (entries != null) : "No entries segment";
                entries.add(value);
                return null;
            }
        });
    ...
    private class EntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        @Override
        public void remove() {
            assert (this.last != null) : "No element to remove";
