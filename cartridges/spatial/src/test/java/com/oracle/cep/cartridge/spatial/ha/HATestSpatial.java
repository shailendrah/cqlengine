 ...
 * the spatial join is done by BinJoin execution operator.
 * 
 * @author santkumk
 *
 */
public class HATestSpatial extends BaseCQLTestCase
{
  public void testSpatial() throws Exception
  {
    ...
    testMetadata.addDestinationMetadata("q1", "outSpatial_q1");
    runFullSnapshotTestBase(new CQLSpatialProcessor(), testMetadata, testSchema);
  }
  
  /**
   * Test Incremental Snapshot for Spatial Relation
   * @throws Exception
   */
  public void testSpatial2() throws Exception
  {
    String testSchema = new Object()
      {}.getClass().getEnclosingMethod().getName();
    String[] setupDDLs = new String[] {
    ...
    String[] tearDownDDLs = new String[] { "drop query q1",
        ...
        "drop view FenceGeomRelation", "drop relation FenceRelation" };
    TestMetadata testMetadata = new TestMetadata(setupDDLs, tearDownDDLs);
    testMetadata.addSourceMetadata("PosStream", "inpSpatial2_PosStream");
    testMetadata.addSourceMetadata("FenceRelation", "inpSpatial2_FenceRelation");
    testMetadata.addSourceType("FenceRelation", SourceType.RELATION);
    testMetadata.addDestinationMetadata("q1", "outSpatial2_q1");
    runJournalSnapshotTestBase(new CQLSpatialProcessor(), testMetadata, testSchema);
  }
   ...
   * @throws Exception
   */
  public void testSpatial3() throws Exception
  {
    String testSchema = new Object()
      {}.getClass().getEnclosingMethod().getName();
    String[] setupDDLs = new String[] {
        ...
        "register relation FenceRelation(id char(100), nCoords int, "
            + "lat1 double, lng1 double, lat2 double, lng2 double, lat3 double, lng3 double, lat4 double, lng4 double, "
            + "lat5 double, lng5 double, lat6 double, lng6 double, lat7 double, lng7 double) is silent",
        "register view FenceGeomRelation(id char(100), geom com.oracle.cep.cartridge.spatial.Geometry) as "
            + "select id, "
            + " com.oracle.cep.cartridge.spatial.Geometry.createGeometry("
            + " com.oracle.cep.cartridge.spatial.Geometry.GTYPE_POLYGON,"
        ...
        "create query q1 as " 
            + "SELECT fence.id as fenceId, pos.id as posId "
            + "FROM  PosGeomStream[rows 15] as pos, FenceGeomRelation as fence "
            + "WHERE contain@spatial(fence.geom,pos.geom,0.001d) = true" };
    ...
    String[] tearDownDDLs = new String[] { "drop query q1",
        ...
        "drop view FenceGeomRelation", "drop relation FenceRelation" };
    TestMetadata testMetadata = new TestMetadata(setupDDLs, tearDownDDLs);
    testMetadata.addSourceMetadata("PosStream", "inpSpatial3_PosStream");
    testMetadata.addSourceMetadata("FenceRelation", "inpSpatial3_FenceRelation");
    testMetadata.addSourceType("FenceRelation", SourceType.RELATION);
    testMetadata.addDestinationMetadata("q1", "outSpatial3_q1");
    runJournalSnapshotTestBase(new CQLSpatialProcessor(), testMetadata, testSchema);
  }
}
