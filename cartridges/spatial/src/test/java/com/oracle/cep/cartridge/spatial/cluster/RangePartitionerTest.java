...
public class RangePartitionerTest {
    private RangePartitionGrid defaultGrid;

    @Before
    public void setUp() throws Exception {
        SpatialCartridge.createInstance(new CartridgeRegistry());
        defaultGrid = new RangePartitionGrid(new double[][]{{-180,-90},{180,90}},400000,400000);
    }

    @Test
    public void testFencePartitions() throws Exception {
        RangePartitionGrid grid = new RangePartitionGrid(new double[][]{{-122.41979060640048,37.781816037088014},{-122.39597357958169,37.79867596046537}},2,2);
        ...
        RangePartitioner partitioner = new RangePartitioner(grid);
        for (String fence : fences) {
            FenceGeom g = FenceGeom.getFenceGeom(fence);
            int[] p = partitioner.partition(g.getGeom());
            plist.put(g.getId(),p);
        }
